...
public class DataSourceUtils {
    ...
    // ds放到缓存中
    private static Map<String, javax.sql.DataSource> dataSourceConfigs = new HashMap<String, javax.sql.DataSource>();
     ...
     * @return
     */
    public static javax.sql.DataSource getDatasource(String dsAlias, DSManager dsManager) {
        DS ds = null;
        try {
     ...
     * @return
     */
    public static javax.sql.DataSource getDatasource(DS ds) {
        javax.sql.DataSource __ds = null;
        if (dataSourceConfigs.containsKey(ds.getAlias())) {
            __ds = dataSourceConfigs.get(ds.getAlias());
        } else {
            //如果ds中填写了tddl的appname信息，则走tddl的逻辑
            if(ds.getTddlAppName()!=null && !"".equals(ds.getTddlAppName().trim())){
                //.dynamicRule(true)
                 TDataSource tds  = TDataSourceBuilder.create()
                        .appName(ds.getTddlAppName())
                        .sharding(false)
                        .build();
                tds.init();
                dataSourceConfigs.put(ds.getAlias(), tds);
                return tds;
            }else{
                //走正常的jdbc
